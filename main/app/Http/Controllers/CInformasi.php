...
class CInformasi extends Controller
{
    ...
    public function by_date_cuaca(Request $request){

        $tanggalRaw = str_replace("/","-",$request['tanggal']);
        $uraiKarakter = explode("-",$tanggalRaw);
        $tanggal = $uraiKarakter[2]."-".$uraiKarakter[0]."-".$uraiKarakter[1];

        $cuaca = DB::table('nelayan_detailcuaca')
            ->join('nelayan_postlaporan','nelayan_postlaporan.id_laporan','=','nelayan_detailcuaca.id_laporan')
            ->join('nelayan_user','nelayan_user.id_user','=','nelayan_postlaporan.id_user')
            ->where('nelayan_postlaporan.jenis_laporan','=',2)
            ->where('nelayan_postlaporan.detail_waktu','LIKE','%'.$tanggal.'%')
            ->get();

        return response()->json($cuaca);
    }

    public function by_date_keadaan_laut(Request $request){
        $tanggalRaw = str_replace("/","-",$request['tanggal']);
        $uraiKarakter = explode("-",$tanggalRaw);
        $tanggal = $uraiKarakter[2]."-".$uraiKarakter[0]."-".$uraiKarakter[1];

        $keadaanLaut = DB::table('nelayan_detailkeadaanlaut')
            ->join('nelayan_postlaporan','nelayan_postlaporan.id_laporan','=','nelayan_detailkeadaanlaut.id_laporan')
            ->join('nelayan_user','nelayan_user.id_user','=','nelayan_postlaporan.id_user')
            ->where('nelayan_postlaporan.jenis_laporan','=',3)
            ->where('nelayan_postlaporan.detail_waktu','LIKE','%'.$tanggal.'%')
        ...
        return response()->json($keadaanLaut);
    }

    public function by_date_kejahatan(Request $request){
        $tanggalRaw = str_replace("/","-",$request['tanggal']);
        $uraiKarakter = explode("-",$tanggalRaw);
        $tanggal = $uraiKarakter[2]."-".$uraiKarakter[0]."-".$uraiKarakter[1];
        ...
        return response()->json($kejahatan);
    }

    public function by_date_detail_tangkapan(Request $request){
        $tanggalRaw = str_replace("/","-",$request['tanggal']);
        $uraiKarakter = explode("-",$tanggalRaw);
        $tanggal = $uraiKarakter[2]."-".$uraiKarakter[0]."-".$uraiKarakter[1];

        $detailTangkapan = DB::table('nelayan_detailtangkapan')
            ->join('nelayan_postlaporan','nelayan_postlaporan.id_laporan','=','nelayan_detailtangkapan.id_laporan')
            ->join('nelayan_user','nelayan_user.id_user','=','nelayan_postlaporan.id_user')
            ->where('nelayan_postlaporan.jenis_laporan','=',5)
            ->where('nelayan_postlaporan.detail_waktu','LIKE','%'.$tanggal.'%')
        ...
        return response()->json($detailTangkapan);
    }
